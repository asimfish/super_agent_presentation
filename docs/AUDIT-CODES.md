...
| `under-sectioned` | 2,000 or more characters of non-code text with fewer than two section markers (headings or paragraphs opening with a bold lead-in sentence). Not applied to `concise-answer`. | Marking each semantic-role boundary with a heading or bold lead-in, on chat surfaces too. |
| `hedge-saturation` | 20 or more hedging markers (`不能`, `无法`, `尚未`, `可能`, `疑似`, `cannot`, `may`, `unclear`, `not verified`, ...) at a density above 7 per 1,000 characters of body text; sections whose heading or lead-in names boundaries, limitations, uncertainty, or risk are excluded from the count. Calibrated so well-edited reports (1–3 per 1,000) and a bare-prompt baseline (about 5) stay silent. | Stating each boundary once in the boundaries section instead of re-hedging every claim in the body. |
...
| Code | Trigger | Clear it by |
|---|---|---|
| `unlabeled-uncertainty` | `±` (or `+/-`) appears but nothing in the report's prose says what it denotes (SD, SEM, CI, IQR, tolerance, `标准差`, `置信区间`); a label that appears only inside an inline code span does not count. One finding at the first use. | Labeling the interval once in the header, caption, or first use, with the run count (Cumming et al. 2007). |
| `threshold-p-value` | `p < 0.05`, `p < 0.01`, `p < 0.1`, `p > 0.05`, or `n.s.`; `p < 0.001` and scientific-notation values such as `p < 0.01e-5` stay legal. A sentence that declares a threshold (`alpha`, `threshold`, `pre-registered`, `阈值`, `预注册`) is exempt. | Reporting the exact value (`p = 0.031`); never reading `p > 0.05` as no effect (ASA 2016). |
...
| `up-to-without-central-tendency` | `up to N×`, `up to N times`, `最高可达 N 倍` while the report's prose contains no geometric mean, median, mean, average, or worst case anywhere; a term that appears only inside an inline code span does not count. | Adding the geometric mean or median and the worst case, and saying when the best case occurs (Heiser). |
| `best-of-n-runs` | `best of 5 runs`, `the best run is reported`, `报告最好的一次`. `Best-of-n sampling` as a method name is not matched, and an immediate denial (`we do not report the best of 5 runs`, `我们不取最好的一次`) is exempt, although a later affirmative selection in the same sentence still warns. | Reporting the median or mean with dispersion over all runs, or stating why best-of-n is the deployment-relevant statistic. |
