...
- **2026-09-12** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square)
  ...
  因为发现有一个模式默认包早已悄悄超限。同日加了**成品范例**（`reportctl
  exemplar 模式名`）：状态更新、实验报告、决策简报、研究想法四个模式各一段目标
  文风的成稿，中英各一，写第一句之前先读；规则描述形式，范例直接给看。每段都在
  测试里按自己的模式过审计。
- **2026-09-07** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square)
  ...
  点出、是否忠于事实清单。拿三份过了零告警审计的修订前旧稿试，第二个模型把
  人工找到的三处推理错误全部标为 blocker，还多找出两处人工漏掉的。同时修掉框架
  在聊天表面上自己造成的两个退步（同事实、同模型对照裸提示发现）：长报告没有
  分节标记、限定词密度是裸版 1.7 倍；新增 `under-sectioned`、`hedge-saturation`
  告警，基线对照表要求差值列。对照的诚实结论：强模型做一次性短报告，裸提示与
  框架接近平手；对 gpt-5.5 框架帮助明显。
